  ...
  <div ref="pageBlock" class="planningCreateMain">
    ...
    >
      <div class="image_progress_group">
        <div class="image_progress_item" v-for="(item, i) in imageResult" v-bind:key="item.id">
          <span>{{ item.name }}</span>
          <el-progress v-if="item.percentage = 50" status="exception" :stroke-width="10" :percentage="item.percentage">
          </el-progress>
          <el-progress v-else-if="item.percentage = 0" :stroke-width="10" :percentage="item.percentage">
          </el-progress>
        </div>
      </div>
