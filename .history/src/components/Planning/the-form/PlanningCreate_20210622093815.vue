...
export default {
  ...
  async created() {
    // 根据页面类型获取数据
    if (~["edit", "detail", "compare", "add"].findIndex((i) => i === this.formType)) {
      this.postPlanning();
    }
  },
  mounted() {
  ...
  methods: {
    postPlanning() {
      postRequest("/distribution_channel/withChildren", this.params).then((res) => {
    ...
    resolveDataToMap(data) {
      const form1 = {
        name: data.name,
        brand: data.brand,
        distribution_channel: ["线下销售渠道", "国美"],
      };
      const form2 = {};
      return { form1, form2 };
    },
