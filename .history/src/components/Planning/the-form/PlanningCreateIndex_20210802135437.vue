  ...
  <div ref="pageBlock" class="planningCreateMain">
    ...
    >
      <div class="image_progress_group">
        <div class="image_progress_item" v-for="(item, i) in formDataMap.imageList" v-bind:key="item.id">
          <!-- <span>{{ item.name }}</span> -->
          <el-progress :stroke-width="10" :percentage="percentage" :format="item.name">
          </el-progress>
        </div>
      </div>
